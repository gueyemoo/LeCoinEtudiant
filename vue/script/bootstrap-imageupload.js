...
(function($) {
    ...
    function isValidImageFile(file, callback) {
        ...
        {
            callback(false, 'Le fichier est trop lourd (max ' + options.maxFileSizeKb + 'kB).');
            return;
        }
        ...
        if ($.inArray(fileExtension, options.allowedFormats) > -1) {
            callback(true, 'Cette image n\'est pas valide.');
        }
        else {
            callback(false, 'Ce type de fichier n\'est pas accepter.');
        }
    }
